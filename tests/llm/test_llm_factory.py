...
def test_create_llm_returns_mock_for_test_env():
    ...
    llm = create_llm(env="test", app_cfg=app_cfg)
    assert isinstance(llm, MockLLMClient)


def test_create_llm_returns_mock_for_mock_provider():
    app_cfg = AppConfig.model_validate(
        {"version": "1.0", "llm": {"provider": "mock", "base_url": "http://localhost:4000"}, "db": {"path": ".data/db/advisor.sqlite"}}
    )
