...
- KISS — ядро оркестратора фокусируется на execution control flow (planner → executor → review → retry). Cross-cutting concerns (persistence, metrics, logging) — за пределами ядра, через hooks. Retry-механизм в ядре, но retry-политики могут быть customizable через конфиг.
...
## 1. Назначение
Проект реализует **vendor-agnostic LLM advisor strategy system**: архитектуру planner–executor с опциональным review loop. Система должна быть:
...
Для каждого шага плана:
  ...
  ↓
  (Опционально) Reviewer проверяет -> возвращает ReviewResult
  ↓
  если reject:
...
- **Если review недоволен — не перепланировать сразу**: сначала retry шага с другим executor/model согласно политике.
...
`RunContext` — единый объект, который создаётся при старте run'а и сопровождает всё выполнение: planner → executor(s) → review → final answer. Это точка консолидации для state, persistence, hooks и retry-логики.
...
- `review_verdict` — review оценил шаг (step_id, approved: bool, feedback)
...
- **Flow-agnostic**: один events-based контейнер работает для Planner-Executor-Reviewer, ReAct, Hub-and-Spoke, динамических flows. Каждый flow определяет свои event-типы, но storage uniform
...
review_verdict = ctx.events |> filter(type='review_verdict') |> last()
```
...
- `ReviewResult` — verdict от Reviewer (approved / issues)
...
Reviewer: структурированный verdict (валидируем Pydantic)
...
- **Reviewer output**: структурированный verdict (`approved: bool`, `feedback: str`), валидируем Pydantic.
...
- **ValidationError** (невалидный JSON от planner/review) → repair-попытка с уточняющим промптом
- **ExecutorError** (сбой в исполнении шага) → retryable с другим executor/model через Router
- **ReviewRejectError** (штатный reject от review) → отдельный путь (Step 3.4): retry шага через Router без перепланирования
...
2) ошибок качества/валидации (review reject, невалидный JSON planner/review)

Ключевая политика:
- review reject → retry того же шага с альтернативным executor/model
...
stateDiagram-v2
  [*] --> Plan
  Plan --> ExecuteStep
  ExecuteStep --> ReviewerCheck
  ReviewerCheck --> NextStep: approve
  ReviewerCheck --> RetryStep: reject
...
- `on_review_verdict(ctx, step, verdict)` — после получения verdict от review
...
Planner-Executor-Reviewer — один flow. Когда потребуется расширение, Flow вводится как абстракция со своим контрактом (участники, логика передачи управления, критерии завершения). Оркестратор становится "flow runner". 
...
Роли (Planner, Executor, Reviewer) — это конфигурации агента (prompt + tools + model alias), а не классы-наследники. Это позволяет добавить новую роль (например, Hub или Spoke) только конфигом, без нового класса.
